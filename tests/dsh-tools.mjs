...
test('released Harness scopes share the complete catalog and keep each tool bound to its calling editor', async () => {
  ...
  try {
    ...
    const first = await run(alpha, 'workspace.info', { file: 'main.typ' })
    ...
    assert.deepEqual(calls.map(({ name, input }) => ({ name, input })), [
      { name: 'tylina', input: { command: 'workspace.info', args: { file: 'main.typ' } } },
    ...
    assert.equal((await run(beta, 'workspace.info', { file: 'main.typ' })).isError, false)
    releaseBeta()
