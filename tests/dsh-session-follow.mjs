...
export async function verifySessionFollowing({ page, frame, home, probeRequest, readMain, external, expect }) {
  ...
  await open.click()
  await page.getByRole('button', { name: /^(固定此工作区|Pin this workspace)$/u }).click()
  await hide.click()
  ...
  await expect(page.getByRole('button', { name: /^(跟随当前会话|Follow the current conversation)$/u })).toHaveAttribute('aria-pressed', 'true')
  await page.getByRole('button', { name: /^(跟随当前会话|Follow the current conversation)$/u }).click()
