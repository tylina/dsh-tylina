...
```sh
dsh plugin --profile web add dsh-tylina@0.4.9
dsh web
```
...
For a newly published release, use `dsh plugin --profile web update dsh-tylina@0.4.9`.
...
The browser edition is **0.4.9**; the published native edition remains **0.4.4** while its matching platform update is being validated. Choose browser WASM or native compilation on the Harness host.
